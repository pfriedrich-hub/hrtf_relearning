...
chirp1 = hrtf.apply(37, chrp)
chirp2 = kemar.apply(339, chrp)
...
def front_back_test(sofa_file='kemar', sound=sound, freefield=False):
    ...
    for c in range(5):
        ...
        print(np.unique(hrtf.sources[cone][:, 0]))
        for s in range(10):
            source = random.choice(cone)
            #  print hrtf.sources[source]
            if np.deg2rad(hrtf.sources[source][0]) < 1:
                direction = 'front'
            else: direction = 'back'
            #  print(direction)
            ...
            if freefield:
                ...
                sound.play()
                choice = (input('Press ''F'' for sounds from the FRONT / ''B'' for sounds from the BACK.'))
