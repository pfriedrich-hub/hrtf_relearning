...
def learning_plot(to_plot='average', path=Path.cwd() / 'data' / 'experiment' / 'master', w2_exclude = ['cs', 'lm', 'lk']):
    ...
    axes[0].set_yticks(numpy.arange(0, 1.2, 0.2), size=13)
    axes[0].set_xticks(days, size=13)
    axes[0].set_ylabel('Elevation Gain', size=13)
    axes[1].set_xticks(days)
    axes[1].set_xlabel('Days', size=13)
    axes[1].set_ylabel('Elevation in degrees', size=13)
    axes[1].legend()

    for y1, y2 in zip(numpy.linspace(.1, 1, 10), numpy.linspace(5, 23, 10)):
        axes[0].axhline(y=y1, xmin=0, xmax=20, color='grey', linewidth=.1)
        axes[1].axhline(y=y2, xmin=0, xmax=20, color='grey', linewidth=.1)
