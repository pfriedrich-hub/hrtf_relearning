...
def learning_plot(to_plot='average', path=Path.cwd() / 'data' / 'experiment' / 'master', w2_exclude = ['cs', 'lm', 'lk']
    ...
    for i, ax_id in enumerate([0, 1, 1]):
        ...
        axes[ax_id].plot([4.95, 5], [m1[5, i], ef[1, i]], c=str(color-w2_dev), ls=(0, (5, 10)), lw=0.8)  # week 1 mold removal
        # week 2
        axes[ax_id].plot([5, 5.05], [ef[1, i], m2[0, i]], c=str(color+w2_dev), ls=(0, (5, 10)), lw=0.8)  # week 2 mold insertion
        axes[ax_id].plot([5.05,  6,  7,  8, 9, 9.95], m2[:6, i], c=str(color+w2_dev), lw=1)  # week 2 learning curve
        axes[ax_id].plot([9.95, 10], [m2[5, i], ef[2, i]], c=str(color+w2_dev), ls=(0, (5, 10)), lw=0.8)  # week 2 mold removal
        # mold 1 adaptation persistence
        axes[ax_id].plot([4.95, 10.05], m1[-2:, i], c=str(color-w2_dev), ls=(0, (5, 10)), lw=1)
        # mold 2 adaptation persistence
        axes[ax_id].plot([9.95, 15], m2[-2:, i], c=str(color+w2_dev), ls=(0, (5, 10)), lw=1)
        # error bars
        if len(subjects) > 1:
            axes[ax_id].errorbar([0, 5, 10], ef[:3, i], capsize=3, yerr=localization_dict['Ears Free']['SE'][:3, i],
                           fmt="o", c=str(color), markersize=3, fillstyle='full',
                                 markerfacecolor='white', markeredgewidth=.5)  # error bar ears free
            axes[ax_id].errorbar([.05, 1, 2, 3, 4, 4.95, 10.05], m1[:7, i], capsize=2, yerr=localization_dict
                            ['Earmolds Week 1']['SE'][:7, i], fmt="o", c=str(color-w2_dev), markersize=3, markeredgewidth=.5)  # err m1
            axes[ax_id].errorbar([5.05,  6,  7,  8, 9, 9.95, 15], m2[:7, i], capsize=2,
                                 yerr=localization_dict['Earmolds Week 2']['SE'][:7, i], fmt="o", c=str(color+w2_dev),
                                 markersize=3, markeredgewidth=.5)  # err m2
    ...
    axes[1].set_xticklabels([0,5,10,15])
    axes[1].set_ylim(4,26)
    axes[1].set_yticks(numpy.arange(5, 26, 5))
    ...
    for ax_id, ax in enumerate(axes):
        ax.annotate(subpl_labels[ax_id], (-.06, 0.95), c='k', weight='bold', xycoords='axes fraction')
    return fig, axes
