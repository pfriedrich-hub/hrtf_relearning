...
def vsi_l_r(main_df, axis):
    if not axis:
        ...
        axis.set_title('left and right ear VSI')
    ...
    for i in range(3):
        ...
        ys = slope * x + intercept
        axis.plot(x, ys, lw=0.4, c=c_list[i])
...
def sp_str_l_r(main_df, axis):
    ...
    if not axis:
        ...
        axis.set_title('left and right ear spectral strength')
    c_list = ['#2668B3', '#F7D724', '#CF1F48']  # triadic color scheme
    axis.scatter(x[0], y[0], marker='.', c=c_list[0], label='Ears Free')
    axis.scatter(x[1], y[1], marker='.', c=c_list[1], label='M1')
    axis.scatter(x[2], y[2], marker='.', c=c_list[2], label='M2')
...
def scatter_vsi_dis_l_r(main_df, axis):
    """
    compute VSI dissimilarity / spectral difference between every possible pair of participants
...
def boxplot_sp_str(main_df, axis):
    x = numpy.zeros((3, len(main_df['subject'])*2))
    efm1_l = main_df['EF M1 spectral strength l'].to_numpy(dtype='float16', na_value=numpy.NaN)
    efm1_r = main_df['EF M1 spectral strength r'].to_numpy(dtype='float16', na_value=numpy.NaN)
    x[0] = numpy.hstack((efm1_l, efm1_r))
    efm2_l = main_df['EF M2 spectral strength l'].to_numpy(dtype='float16', na_value=numpy.NaN)
    efm2_r = main_df['EF M2 spectral strength r'].to_numpy(dtype='float16', na_value=numpy.NaN)
    x[1] = numpy.hstack((efm2_l, efm2_r))
    m1m2_l = main_df['M1 M2 spectral strength l'].to_numpy(dtype='float16', na_value=numpy.NaN)
    m1m2_r = main_df['M1 M2 spectral strength r'].to_numpy(dtype='float16', na_value=numpy.NaN)
    x[2] = numpy.hstack((m1m2_l, m1m2_r))
    if not axis:
        fig, axis = plt.subplots(1, 1)
        # axis.set_xlabel('Left Ear VSI Dissimilarity')
...
def permutation_vsi_dis(main_df, axis):
    """
