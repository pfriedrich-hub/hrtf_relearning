...
sofa_path_target = os.getcwd() + '/data/hrtf/sofa/aachen_database/MRT01.sofa'
sofa_path_template = os.getcwd() + '/data/hrtf/sofa/aachen_database/MRT01.sofa'
sig_path = os.getcwd() + '/data/sounds/1s_pinknoise_44100.wav'
fig_savepath = os.getcwd() + '/data/plot'
shutup = True
do_dtf = False
is_save = True
...
def baumgartner2014(sofa_path_target, sofa_path_template, sig_path, shutup, do_dtf, fig_savepath, is_save=False):
    ...
    file_name_with_extension = os.path.basename(sofa_path_target)
    # Remove the file extension to get the file name without the file type
    ...
    # step 4 - distances between target(s?) and templates
    sigma = eq_4(template_grad, target_grad, shutup)
    # step 5 - response probabilities
    gamma = 6  # Example value for gamma
    S = 0.7  # Example value for S
    si = eq_5(sigma, S, gamma, shutup=False)
    # step 6 - binaural weighting
    lat = 0
    bwcoef = 13
    sibin = eq_6(si, bwcoef, lat, shutup=False)
    # step 7 - sensorimotor mapping
    ...
    ri, rang = eq_7(sibin, coardinate_median_plane, kv_rangsamp, lat, mrsmsp, flags_do_regular, shutup=False)
    # step 8 - normalization to probabilities
    pdf = eq_8(ri)
    if is_save:
        plot_pdf_matrix(pdf, rang, coardinate_median_plane, file_name_without_extension, os.getcwd() + '/data/plot', shutup)
...
def plot_pdf_matrix(pdf, rang, coardinate_median_plane, title_name, fig_savepath, shutup):
    ...
    plt.savefig(fig_savepath + "/median_localization_" + title_name + ".png")
    if not (shutup):
        plt.show()
    else:
        plt.close()
