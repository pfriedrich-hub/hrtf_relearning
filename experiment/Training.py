...
def head_tracking(distance, target, osc_client, sensor_state):
    ...
    sensor_state[0] = 1  # flag for completed sensor init
    ...
    for az, ele in zip(numpy.linspace(-30, target[0], 1000), numpy.linspace(-30, target[1], 1000)):
        if sensor_state[0] == 2:  # deactivate headpose update between trials?
...
def pulse_stream(pulse_interval, binsim_state):
    ...
    while not binsim.stream._is_running:
        time.sleep(0.1)
    binsim_state[0] = 1  # flag for binsim init
    ...
    while True:
        if binsim_state[0] == 1:  # Mute sound, stop updating pulse interval
            smooth_transition(0)
            interval = None
        else:  # play
            interval = pulse_interval[0]
            if interval < 5:  # continuous sound
...
def play_session(game_time, trial_time, target_size, target_time, az_range, ele_range):
    ...
    target = mp.Array("i", [0, 0])  # send current target from get_target to sensor_tracking
    distance = mp.Array("f", [0])  # update distance and send from sensor_tracking to play_trial
    pulse_interval = mp.Array("i", [-1]) # update interval and send from play_trial to pulse_stream, initially muted
    ...
    pulse_worker = mp.Process(target=pulse_stream, args=(distance, binsim_state))
    pulse_worker.start()
    ...
    while time.time() - start_time < game_time:
        ...
        binsim_state[0] = 2  # start pulse train
        trial_worker.start()

        # stop
        trial_worker.join()
        sensor_state[0] = 1  # start sensor tracking
        binsim_state[0] = 1
        trial_worker.terminate()
        scores.append(score[0])
        print(f'Trial complete: {score[0]} points')
