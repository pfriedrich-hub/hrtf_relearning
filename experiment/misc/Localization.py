...
subject_id = 'test'
hrtf_filename ='KU100_HRIR_L2702'
slab.set_default_samplerate(slab.HRTF(data_dir / 'hrtf' / 'sofa' / f'{hrtf_filename}.sofa').samplerate)


class Localization:
    def __init__(self, subject_id, hrtf_filename):
        # metadata
        self.filename = subject_id + '_' + date
        self.subject = Subject(subject_id)
        self.sources = slab.HRTF(data_dir / 'hrtf' / 'sofa' / hrtf_filename).sources

        # make trial sequence
        self.sequence = self._make_sequence(150, (-52.5, 52.5), (-37.5, 37.5), 35)

        # init pybinsim
        self.osc_client = self._init_osc_client()
        self.binsim = self._init_pybinsim(hrtf_filename)
    ...
    @staticmethod
    def _init_pybinsim(self, filename):
    ...
    def play_stim(self):
        ...
        stim.ramp('both', 0.01)
        stim.write(data_dir / 'wav' / f'{self}' / 'sounds' / 'noise_burst.wav')
        binsim.config.configurationDict['loudnessFactor'] = .5
        osc_client.send_message('/pyBinSimFile', str(data_dir / 'wav' / f'{filename}' / 'sounds' / 'noise_burst.wav'))
        time.sleep(stim.duration)
        binsim.config.configurationDict['loudnessFactor'] = 0
        osc_client.send_message('/pyBinSimFile', str(data_dir / 'wav' / f'{filename}' / 'sounds' / 'silence.wav'))
