...
raw_recording = slab.Sound(data=numpy.mean(recs, axis=0))
...
bandwidth = 1 / 50
alpha = 1.0
filter_bank = slab.Filter.equalizing_filterbank(signal.channel(0), raw_recording, length=freq_bins, low_cutoff=low_cutoff,
...
# fig.suptitle(
#     'Calibration for dome speaker column at %.1f° azimuth. \n Difference in power spectrum' % speakers[0].azimuth,
#     fontsize=16)
#
# # save equalization
...
# for az in azimuthal_angles:
#     speaker_list.append((speaker_table[speaker_table[:, 1] == az][:, 0]).astype('int'))
# speaker_list[3] = numpy.delete(speaker_list[3],
#                                [numpy.where(speaker_list[3] == 19), numpy.where(speaker_list[3] == 27)])
