...
def stim_response_plot(sub_id='vk', figsize=(14, 7), conditions = ['Ears Free', 'Earmolds Week 1'],
    ...
    for axis, condition in zip(axes, conditions):
        ...
        for idx, target in enumerate(elevation_ticks):
            ...
            # mean
            axis.hlines(mean, target-width/2, target+width/2, colors='0.5', )
            # RMSE
            axis.plot([target, target], [mean, target], c='0.5', linestyle='-', linewidth=lw, label='RMSE')
            # target
            axis.hlines(target, target-width/2, target+width/2, colors='0', )
            # SD
            axis.bar([target], height=2*sd, bottom=mean-sd, color='0.8',
        ...
        axis.plot(x, y, c='0', linewidth=lw, linestyle='--', label='EG')
    ...
    handles = handles[-2:]
    labels = labels[-2:]
    handles.append(matplotlib.patches.Rectangle((1,1), width=0, height=0, color='0.8', ))
    labels.append('SD')
