...
plot_path = Path('/Users/paulfriedrich/Desktop/HRTF relearning/legacy/MSc/Thesis/Results/figures')
conditions = ['Ears Free', 'Earmolds Week 1', 'Earmolds Week 2']
main_df = get_df.main_dataframe(path, processed_hrtf=True)
measures = ['EG', 'RMSE ele', 'SD ele', 'AG', 'RMSE az', 'SD az']
...
for m_idx, measure in enumerate(measures):
    ...
    persistence_results['M1'][measure]['p'] = scipy.stats.wilcoxon(m1d5[:, m_idx], m1d10[:, m_idx], alternative=alternative, nan_policy='omit')
    ...
    persistence_results['M2'][measure]['p'] = \
        scipy.stats.wilcoxon(m2d5[:, m_idx], m2d10[:, m_idx], alternative=alternative, nan_policy='omit')
