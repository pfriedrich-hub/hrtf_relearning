...
def spectral_overview(main_df, axes=None, cbar_axes=None, zlim=None, figsize=(21, 5), return_n=False):
    """
    """
    ...
    params = {'font.family':'Helvetica', 'xtick.labelsize': fs, 'ytick.labelsize': fs, 'axes.labelsize': fs,
              ...
              'xtick.major.size': 2, 'axes.linewidth': lw, 'axes.titlesize': fs}
    ...
    if not axes:
        fig, axes = plt.subplots(1, 6, figsize=(fig_width, fig_height), dpi=dpi,
                                 gridspec_kw={'width_ratios': [14, 1, 14, 1, 14, 1]}
                                 ,constrained_layout=True)
    if not cbar_axes:
        cbar_axes = [axes[1], axes[3], axes[5]]
    ...
    plot_spectral_change_p(main_df, 0, threshold, bandwidth,  return_n, axes[2], True,
                           cbar_axis=cbar_axes[1])
    plot_spectral_change_p(main_df, 1, threshold, bandwidth, return_n, axes[4], True,
                           cbar_axis=cbar_axes[2])
    ...
    axes[2].annotate('B', size=number_size, xy=(.008, .89), c='w', weight='bold', xycoords='axes fraction'
                     , path_effects=[pe.withStroke(linewidth=1, foreground='k')])
    axes[4].annotate('C', size=number_size, xy=(.008, .89), c='w', weight='bold', xycoords='axes fraction'
                     , path_effects=[pe.withStroke(linewidth=1, foreground='k')])
    plt.show()
...
def ear_mod_images(main_df, subject, chan=0, figsize=(6.5, 13), bandwidth=(4000, 16000), n_bins=None, axis=None,
    ...
    params = {'font.family':'Helvetica', 'axes.spines.right': True, 'axes.spines.top': True,
    ...
    fig, axes = plt.subplots(1, 4, figsize=(fig_width, fig_height), dpi=dpi,
                             gridspec_kw={'width_ratios': [8.5, 8.5, 8.5, .5]}
                             ,constrained_layout=True)
    # for ax in axes[:3]:
    #     ax.set_box_aspect(1)
    try:
    ...
    # titles
    axes[0].set_title('Free ears')
    axes[1].set_title('Mold 1')
    axes[2].set_title('Mold 2')
    # subplot numbering
    number_size = plt.rcParams.get('axes.titlesize')
    labels = ['B', 'C', 'D']
    for i, ax in enumerate(axes[:3]):
        ax.annotate(labels[i], path_effects=[pe.withStroke(linewidth=1, foreground='k')],
                    size=number_size, xy=(.014, .89), c='w', weight='bold', xycoords='axes fraction')
    # axes[0].annotate('A', size=number_size, xy=(.014, .87), c='k', weight='bold', xycoords='axes fraction')
    # axes[1].annotate('B', size=number_size, xy=(.014, .87), c='k', weight='bold', xycoords='axes fraction')
    # axes[2].annotate('C', size=number_size, xy=(.014, .87), c='k', weight='bold', xycoords='axes fraction')
...
def hrtf_image(hrtf, bandwidth=(4000, 16000), n_bins=None, axis=None, chan=0, z_min=None, z_max=None, cbar=True,
    ...
    # plot contour lines between levels
    axis.contour(contour, colors='k', linestyles='solid', levels=numpy.linspace(z_min, z_max, 10), linewidths=.5)
...
def plot_spectral_change_p(main_df, condition, threshold, bandwidth=(4000, 16000), return_n=False,
                           axis=None, cbar=True, cbar_axis=None):
    change_p, _, n = stats_df.spectral_change_p(main_df, threshold, bandwidth, return_n)
    ...
    ticks = [str(x) for x in (numpy.arange(bandwidth[0], bandwidth[1] + 1, bandwidth[0]) / 1000).astype('int')]
    if not axis:
        fig, axis = plt.subplots()
    else:
        fig = axis.get_figure()
    ...
    if return_n:
        cbar_levels = numpy.linspace(0, n, 50)
    else:
        cbar_levels = numpy.linspace(0, 1, 50)
    # cmap = plt.get_cmap('cmr.rainforest_r')
    # cmap = 'RdYlBu_r'
    # cmap = plt.get_cmap('viridis_r')
    ...
    # filtered = change_p[:, freq_idx]
    # Resample your data grid by a factor of 3 using cubic spline interpolation.
    # filtered = scipy.ndimage.zoom(change_p[:, freq_idx], 3.4)

    from scipy.ndimage import gaussian_filter  # filter before plotting
    filtered = gaussian_filter(change_p[:, freq_idx], sigma=.7)
    # filtered = change_p[:, freq_idx]

    # plot contours around % of participants
    thresh = .7
    if return_n:
        axis.contour(frequencies[freq_idx], elevations, filtered, colors='k', linestyles='solid',
                     levels=numpy.array([0., n*thresh, 14.]), linewidths=.5)
    else:
        axis.contour(frequencies[freq_idx], elevations, filtered, colors='k', linestyles='solid',
                    levels=numpy.array([0. ,1*thresh, 1. ]), linewidths=.5)

                 # levels=numpy.linspace(0,1,4), linewidths=.5)
    ...
    if cbar:
        ...
        if return_n:
            cbar_ticks = [int(0), int(numpy.ceil(n/2)), int(n)]
        else:
        ...
        if return_n:
            cbar.set_ticklabels(['0', str(int(numpy.ceil(n/2))), str(int(n))])
            cbar_axis.set_title('n')
        else:
            cbar.set_ticklabels(['0', '0.5', '1'])
            cbar_axis.set_title('p')
        cbar_axis.tick_params(axis='both', direction="in", bottom=True, top=True, left=True, right=True,
                              width=1, length=1)
        y_ticks = cbar_axis.set_yticks(cbar_ticks)
        y_ticks[0]._apply_params(width=0)
        y_ticks[-1]._apply_params(width=0)
    return axis
...
def plot_hrtfs(hrtf_df, condition='Ears Free'):
    ...
    for subj_idx, hrtf in enumerate(hrtf_list):
        ...
        plt.savefig(f'/Users/paulfriedrich/Desktop/hrtf_relearning/data/figures/hrtf/{subject_id}')


def subj_hrtf_vsi(hrtf_df, to_plot='all', condition='Ears Free', bands=None):
    """
    plot hrtf and vsi across bands for all subjects
    """
    if not bands:
...
#     
#     plot hrtf and vsi dissimilarity across bands for all subjects
#     
#     if not bands:
#         bands = [(4000, 5700), (5700, 8000), (8000, 11300), (11300, 16000)]  # to modify
#     if to_plot != 'all':
