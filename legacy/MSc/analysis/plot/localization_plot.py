...
def response_evolution(to_plot='average', axis=None, figsize=(14,8), path=Path.cwd() / 'MSc' / 'data' / 'experiment' / 'master'):
    ...
    params = {'font.family':'Helvetica', 'xtick.labelsize': fs, 'ytick.labelsize': fs, 'axes.labelsize': fs,
              ...
              'xtick.major.size': 2, 'axes.linewidth': lw, 'axes.titlesize': fs, 'axes.spines.right': False,
    ...
    if not axis:
        fig, axes = plt.subplots(2, 4, sharex=True, sharey=True, figsize=(in_width, in_height),
                                 subplot_kw=dict(box_aspect=2/3), layout='constrained', dpi=dpi)
        fig.subplots_adjust(left=None, bottom=None, right=None, top=None, hspace=0.05, wspace=0.05)
    fig.supxlabel("Response Azimuth (deg)", size=fs)
    fig.supylabel("Response Elevation (deg)", size=fs)
    # M1
    ...
    axes[0,3].set_ylabel('Mold 1')
    axes[0,3].yaxis.set_label_position("right")
    axes[1,3].set_ylabel('Mold 2')
...
def plot_response_pattern(sequence, axis=None, show_single_responses=True, labels=True, custom_limits=None):
    # retrieve data
    ...
    if show_single_responses:
        axis.scatter(responses[:, 0], responses[:, 1], s=3, edgecolor='grey', linewidth=.5, facecolor='none')
    ...
    if custom_limits:
        ...
        axis.set_ylim(custom_limits[0], custom_limits[1])
        axis.set_xlim(custom_limits[2], custom_limits[3])
...
def sub_response_evolution(to_plot='ll', axis=None, figsize=(14,8), path=Path.cwd() / 'MSc' / 'data' / 'experiment' / 'master'):
    """ plot localization free, 1st vs last day of molds and persistence """
    ...
    fig.supxlabel("Response Azimuth (deg)", size=fs)
    fig.supylabel("Response Elevation (deg)", size=fs)
    custom_limits = [-52,52,-62,62]
    # M1
    plot_response_pattern(efd0, axes[0,0], show_single_responses=True, labels=False, custom_limits=custom_limits)
    plot_response_pattern(m1d0, axes[0,1], show_single_responses=True, labels=False, custom_limits=custom_limits)
    plot_response_pattern(m1d5, axes[0,2], show_single_responses=True, labels=False, custom_limits=custom_limits)
    plot_response_pattern(m1d10, axes[0,3], show_single_responses=True, labels=False, custom_limits=custom_limits)
    # M2
    plot_response_pattern(efd5, axes[1,0], show_single_responses=True, labels=False, custom_limits=custom_limits)
    plot_response_pattern(m2d0, axes[1,1], show_single_responses=True, labels=False, custom_limits=custom_limits)
    plot_response_pattern(m2d5, axes[1,2], show_single_responses=True, labels=False, custom_limits=custom_limits)
    plot_response_pattern(m2d10, axes[1,3], show_single_responses=True, labels=False, custom_limits=custom_limits)
    # plot_response_pattern(efd10, axes[2,2], show_single_responses=False, labels=False)
