...
from MSc import head_tracking as motion_sensor
...
def hrtf_training(time_limit, t_max=500, target_size=5, target_time=0.5):
    ...
    offset = motion_sensor.calibrate_pose(sensor, 20)
    # loop over trials
    end = False
    game_time = time.time()
...
def play_trial(speaker_id):
    ...
    # pose = get_pose()  # set initial isi based on pose-target difference
    distance_to_pulse()
    freefield.play(kind='zBusA', proc='all')   # start playing pulse train
    count_down = False
    pose_list = []
    while True:
        # pose = get_pose()  # set isi and return pose-target distance
        # pose_list.append(pose)
        # pose = numpy.mean(pose_list[slice(-10, None)], axis=0)
        dist = distance_to_pulse(pose, target)
        if all(pose):
