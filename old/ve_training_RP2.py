...
def training():
    ...
    variables = {'target': None, 'score': 0, 'game over': False, 'scores':[]}
...
def training_session():
    variables['game over'], variables['score'], variables['prep_time'] = False, 0, 0  # reset game parameters
    variables['game_start'] = time.time()  # get start time
    while not variables['game over']:  # loop over trials until end time has passed
    ...
    variables['scores'].append(variables['score'])
    print(f'Game {len(variables["scores"])}: {variables["score"]} points')
