...
ear canal with microphones mounted in anthropometric earplugs of the PIRATE
design
% \citep{denk_pirate_2019}
. Logarithmic sweeps of 200\,ms duration (120\,Hz to 22\,kHz, 85\,dB SPL) were
...
each was divided by the reference response of the same loudspeaker, leaving
the directional transfer function (DTF). This differs from our earlier work
% \citep{friedrich_adaptation_2025}
, in which the non-directional component was estimated as the average over all
measured transfer functions. A dedicated reference is used here because only
the midline is measured: an average over the measured set would be an average
over the very directions that are subsequently compared, and would remove
part of the elevation dependence it is meant to preserve. The quotient was
onset-aligned, windowed with a 2.5\,ms Hann window and truncated to 512
samples.
...
% - MICROPHONE MODEL IS A PLACEHOLDER. The PIRATE cite covers the earplug, not
%   the transducer. State the actual microphone (make, model, manufacturer) and
%   whether the plugs are individually fitted or come from a size series. The
%   old PUM-3046L-R sentence has been REMOVED -- do not let it drift back in if
%   the current mics are different.
